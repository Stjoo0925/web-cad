...
// block-manager.ts 파일이 존재해야 함
test("block-manager.ts 파일이 존재해야 함", async () => {
  const filePath = path.resolve(__dirname, "../packages/core/src/documents/block-manager.ts");
  const exists = await fs.access(filePath).then(() => true).catch(() => false);
  assert.ok(exists, "block-manager.ts 파일이 존재해야 함");
});
