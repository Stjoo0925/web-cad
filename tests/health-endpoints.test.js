...
// health-server.js 파일이 존재해야 함
test("health-server.js 파일이 존재해야 함", async () => {
  const filePath = path.resolve(__dirname, "../apps/server/src/health-server.js");
  const exists = await fs.access(filePath).then(() => true).catch(() => false);
  assert.ok(exists, "health-server.js 파일이 존재해야 함");
});
