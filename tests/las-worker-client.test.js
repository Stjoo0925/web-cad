...
// las-worker-client.js 파일이 존재해야 함
test("las-worker-client.js 파일이 존재해야 함", async () => {
  const filePath = path.resolve(__dirname, "../packages/core/src/point-cloud/las-worker-client.js");
  const exists = await fs.access(filePath).then(() => true).catch(() => false);
  assert.ok(exists, "las-worker-client.js 파일이 존재해야 함");
});
