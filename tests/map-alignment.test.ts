...
// map-alignment.ts 파일이 존재해야 함
test("map-alignment.ts 파일이 존재해야 함", async () => {
  const filePath = path.resolve(__dirname, "../packages/core/src/geometry/map-alignment.ts");
  const exists = await fs.access(filePath).then(() => true).catch(() => false);
  assert.ok(exists, "map-alignment.ts 파일이 존재해야 함");
});
