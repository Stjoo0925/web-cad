...
// document-repository.ts 파일이 존재해야 함
test("document-repository.ts 파일이 존재해야 함", async () => {
  const filePath = path.resolve(__dirname, "../apps/server/src/db/document-repository.ts");
  const exists = await fs.access(filePath).then(() => true).catch(() => false);
  assert.ok(exists, "document-repository.ts 파일이 존재해야 함");
});
