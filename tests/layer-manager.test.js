...
// 레이어 관리자 파일이 존재해야 함
test("layer-manager.js 파일이 존재해야 함", async () => {
  const filePath = path.resolve(__dirname, "../packages/core/src/documents/layer-manager.js");
  const exists = await fs.access(filePath).then(() => true).catch(() => false);
  assert.ok(exists, "layer-manager.js 파일이 존재해야 함");
});
