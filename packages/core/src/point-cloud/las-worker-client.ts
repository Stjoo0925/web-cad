...
export interface LasProcessParams {
  buffer: ArrayBuffer;
  assetId?: string;
  documentId?: string;
}

export interface LasProcessResult {
  requestId: string;
  assetId: string | null;
...
export interface ProgressCallback {
  (data: { percent: number; requestId: string }): void;
}

export interface CompleteCallback {
  (data: LasProcessResult): void;
}
...
export interface LasWorkerClientOptions {
  worker?: Worker;
...
export interface LasWorkerClient {
  processLas(params: LasProcessParams): Promise<LasProcessResult>;
  cancel(): void;
}
 ...
 *
 * @param options - 옵션
 * @param options.worker - Web Worker 인스턴스 (선택적, 없으면 simulated 모드)
 * @param options.onProgress - 진행률 콜백 (percent: 0~100)
 * @param options.onComplete - 완료 콜백 (result: 파싱 결과)
 * @param options.onError - 오류 콜백 (error: Error 객체)
 * @returns 워커 클라이언트 인스턴스
 */
export function createLasWorkerClient(options: LasWorkerClientOptions = {}): LasWorkerClient {
   ...
   * @param params.buffer - LAS/LAZ 파일 버퍼
   * @param params.assetId - 애셋 ID
   * @param params.documentId - 문서 ID
   * @returns 처리 결과
   */
  async function processLas({ buffer, assetId, documentId }: LasProcessParams): Promise<LasProcessResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const handleMessage = (event: MessageEvent) => {
        const { type, requestId: respReqId, data, error, percent } = event.data as {
          type: string;
          requestId: string;
          data?: LasProcessResult;
          error?: string;
          percent?: number;
        };
  ...
  async function simulateProcess(params: {
    ...
    documentId?: string;
    requestId: string;
  }): Promise<LasProcessResult> {
    return new Promise((resolve) => {
      let percent = 0;
      const interval = setInterval(() => {
        percent += 20;
        onProgress?.({ percent: Math.min(percent, 90), requestId: params.requestId });
        if (percent >= 80) {
          clearInterval(interval);

          // 메타데이터만 추출 (실제 디코딩은 simulated)
          const result: LasProcessResult = {
 ...
 */
export function validateWorkerMessage(message: unknown): ValidationResult {
  const errors: string[] = [];
  if (!message) {
