...
// health-server.ts command가 health 서비스에 있어야 함
test("health-server.ts command가 health 서비스에 있어야 함", async () => {
  const content = await readCompose();
  assert.ok(content.includes("health-server.ts"), "health-server.ts command가 health 서비스에 있어야 함");
});
