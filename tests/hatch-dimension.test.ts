...
// hatch-manager.ts 파일이 존재해야 함
test("hatch-manager.ts 파일이 존재해야 함", async () => {
  const filePath = path.resolve(__dirname, "../packages/core/src/documents/hatch-manager.ts");
  const exists = await fs.access(filePath).then(() => true).catch(() => false);
  assert.ok(exists, "hatch-manager.ts 파일이 존재해야 함");
});
...
test("치수 생성 함수가 있어야 함", async () => {
  ...
  if (exists) {
    ...
    const dmPath = path.resolve(__dirname, "../packages/core/src/documents/hatch-manager.ts");
