...
// point-cloud-manifest.ts 파일이 존재해야 함
test("point-cloud-manifest.ts 파일이 존재해야 함", async () => {
  const filePath = path.resolve(__dirname, "../packages/core/src/point-cloud/point-cloud-manifest.ts");
  const exists = await fs.access(filePath).then(() => true).catch(() => false);
  assert.ok(exists, "point-cloud-manifest.ts 파일이 존재해야 함");
});
