...
// dxf-parser.js 파일이 존재해야 함
test("dxf-parser.js 파일이 존재해야 함", async () => {
  const filePath = path.resolve(__dirname, "../packages/core/src/documents/dxf-parser.js");
  const exists = await fs.access(filePath).then(() => true).catch(() => false);
  assert.ok(exists, "dxf-parser.js 파일이 존재해야 함");
});
