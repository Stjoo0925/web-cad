...
// entity-commands.ts 파일이 존재해야 함
test("entity-commands.ts 파일이 존재해야 함", async () => {
  const filePath = path.resolve(__dirname, "../packages/core/src/documents/entity-commands.ts");
  const exists = await fs.access(filePath).then(() => true).catch(() => false);
  assert.ok(exists, "entity-commands.ts 파일이 존재해야 함");
});
